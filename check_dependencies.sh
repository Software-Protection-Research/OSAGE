...
echo "TIGRESS_HOME: $tigress_home_4_0_9"
echo "TIGRESS_PROG: $tigress_prog_4_0_9"
...
if [ -n "${tigress_versions}" ]; then
    while read -r tigress_version; do
        ...
        if [ -z "$tigress_home" ]; then
            ERROR_EXIT "Could not get the home $tigress_home_name for tigress version ${tigress_version} from the config."
        fi
        ...
        if [ ! -f "$tigress_prog" ]; then
            ERROR_EXIT "Tigress executable not found at ${tigress_prog}."
        fi
        export TIGRESS_HOME="$tigress_home"
        echo "Executing: $tigress_prog --version"
        if ! $tigress_prog --version 1>/tmp/tigress_output.log 2>/tmp/tigress_error.log; then
            echo "Tigress output:"
            cat /tmp/tigress_output.log
            echo "Tigress error:"
            cat /tmp/tigress_error.log
