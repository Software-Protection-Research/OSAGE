...
first_asset=$(head -n 1 /in/${sample}.metadata.assets.functions.txt)

# Read the recipe file
args_recipe=$(cat /recipe/${recipe}.arg)

# Replace placeholder for InitEntropy (only first function)
args_recipe=$(echo "$args_recipe" | \
  sed "/--Transform=InitEntropy/{N;s/--Functions=OSAGE_ASSET_PLACEHOLDER_OSAGE/--Functions=main/}")
args_recipe=$(echo "$args_recipe" | \
  sed "/--Transform=InitOpaque/{N;s/--Functions=OSAGE_ASSET_PLACEHOLDER_OSAGE/--Functions=main/}")
args_recipe=$(echo "$args_recipe" | \
  sed "/--Transform=InitImplicitFlow/{N;s/--Functions=OSAGE_ASSET_PLACEHOLDER_OSAGE/--Functions=main/}")
# Replace placeholder for other transforms (all functions)
args_recipe=$(echo "$args_recipe" | \
  sed "s/--Functions=OSAGE_ASSET_PLACEHOLDER_OSAGE/--Functions=${assets}/g")
